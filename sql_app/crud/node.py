...
async def create(db: Session, node: schemas.NodeCreate, system_id: int, user: schemas.User):
...
async def get_by_id(db: Session, node_id: int):
...
async def get_all(db: Session, system_id: int): 
...
async def delete(system_id: int,  node_id: int, user: schemas.User , db: Session ):
...
async def update(system_id: int,  node_id: int, node: schemas.NodeCreate , db: Session, user: schemas.User):
    system = await system_selector(system_id=system_id, user=user, db=db)
