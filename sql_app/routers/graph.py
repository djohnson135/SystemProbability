...
async def create_graph(system_id: int, node_id: int, graph: GraphCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_graphs( node_id : int,  db: Session = Depends(get_db)):
    return await crud_graph.get_graphs(node_id = node_id,  db=db)

@router.delete("/{graph_id}", status_code=204)
async def delete_graph(system_id: int, node_id: int, graph_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_graph(system_id: int, node_id : int, graph_id: int, graph: GraphCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
