...
async def get_nodes(system_id:int, db: Session = Depends(get_db)):
    return await crud_node.get_nodes(system_id = system_id,  db=db)


@router.post("/",response_model= Node)
async def create_node(system_id: int, node: NodeCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return await crud_node.create_node(db=db, node=node,system_id=system_id, user=user)
    
@router.delete("/{node_id}", status_code=204)
async def delete_node(system_id: int,  node_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    await crud_node.delete_node(system_id=system_id, user=user, node_id = node_id, db=db)
    return {"Message", "Successfully Deleted"}
    
@router.put("/{node_id}", status_code=200)
async def update_node(system_id: int, node_id: int, node: NodeCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
